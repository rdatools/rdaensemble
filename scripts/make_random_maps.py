...
for xx, districts_by_type in DISTRICTS_BY_STATE.items():
    for plan_type, ndistricts in districts_by_type.items():
        ...
        command: str = (
            f"scripts/random_map.py --state {xx} --plantype {plan_type} --roughlyequal {roughly_equal} --data ../rdabase/data/{xx}/{xx}_2020_data.csv --shapes ../rdabase/data/{xx}/{xx}_2020_shapes_simplified.json --graph ../rdabase/data/{xx}/{xx}_2020_graph.json --output temp/{prefix}_random_plan.csv --log temp/{prefix}_random_log.txt --no-debug"
        )
        print(command)
        # os.system(command)
